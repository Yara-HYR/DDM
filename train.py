...
from agent.cross_pre_agent_incon import CrossPreAgent_incon
...
def parse_args():
    ...
    parser.add_argument('--agent', default='deepmdp', type=str, choices=['baseline', 'bisim', 'deepmdp','have','have_con','dyac','dyac_action','sac','svea','drq','dyac_local','glve','lve','cross_pre','cross_pre_incon','cross_pre_action'])
    # gpu
    ...
    # environment
    parser.add_argument('--work_dir', default='/data/XXXX-2/logs/carla-vendors/', type=str)
    ...
    parser.add_argument('--encoder_type', default='pixelCat', type=str,
                        )
...
def make_env(args, device):
    if args.suit == 'carla':
        ...
        env = FrameStack(env, k=args.frame_stack, type=args.perception_type)
        eval_env = env

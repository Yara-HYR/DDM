...
def get_dim(encoder_type, feature_dim):
    ...
    elif encoder_type == "pixelCat" or  encoder_type == "pixelEFNet" or  encoder_type == "pixelCrossFusion" or encoder_type == "pixelCat2":
        fff_dim = feature_dim*2
    else:
        fff_dim = feature_dim
    return fff_dim
...
class Critic(nn.Module):
    ...
    def forward(self, obs, action, detach_encoder=False):
        ...
        final_fea, _ = self.encoder_q(obs, detach=detach_encoder)
...
class InverseDynamics(nn.Module):
    def __init__(self, action_shape, hidden_dim):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(2*encoder_feature_dim, hidden_dim), nn.ReLU(),

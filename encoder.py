...
class PixelEncoderCarla098_rgb(PixelEncoder):
    """Convolutional encoder of pixels observations."""
    def __init__(self, obs_shape, feature_dim, num_layers=4, num_filters=32, stride=1):
        super(PixelEncoder, self).__init__()
        # print(obs_shape[0])

        # assert len(obs_shape[0]) == 3
        ...
        self.convs = nn.ModuleList()
        self.convs.append(nn.Conv2d(obs_shape[0][0], 64, 5, stride=2))
        self.convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.convs.append(nn.Conv2d(64, 64, 3, stride=2))
...
class PixelEncoderCarla098_dvs(PixelEncoder):
    """Convolutional encoder of pixels observations."""
    def __init__(self, obs_shape, feature_dim, num_layers=4, num_filters=32, stride=1):
        super(PixelEncoder, self).__init__()
        ...
        self.convs = nn.ModuleList()
        self.convs.append(nn.Conv2d(15, 64, 5, stride=2))
        self.convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.convs.append(nn.Conv2d(64, 64, 3, stride=2))
...
class pixelCat(nn.Module):
    ...
    def __init__(self, obs_shape, feature_dim, num_layers=2, num_filters=32, stride=1):
        ...
        self.feature_dim = feature_dim
        self.num_layers = num_layers
        out_dims = 36#34*34  # 3 cameras

        self.rgb_head_convs = nn.ModuleList()
        self.rgb_head_convs.append(nn.Conv2d(3 * 3, 64, 5, stride=2))
        self.rgb_head_convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.rgb_head_convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.rgb_head_convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.rgb_head_convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.rgb_fc = nn.Linear(64 * out_dims, self.feature_dim)
        self.rgb_ln = nn.LayerNorm(self.feature_dim)

        self.rgb_fc_incon = nn.Linear(64 * out_dims, self.feature_dim)
        self.rgb_ln_incon = nn.LayerNorm(self.feature_dim)

        self.dvs_head_convs = nn.ModuleList()
        self.dvs_head_convs.append(nn.Conv2d(5 * 3, 64, 5, stride=2))
        self.dvs_head_convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.dvs_head_convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.dvs_head_convs.append(nn.Conv2d(64, 64, 3, stride=2))
        self.dvs_head_convs.append(nn.Conv2d(64, 64, 3, stride=2))
        ...
        self.dvs_fc_incon = nn.Linear(64 * out_dims, self.feature_dim)
        self.dvs_ln_incon = nn.LayerNorm(self.feature_dim)
    ...
    def forward_conv(self, obs):
        ...
        rgb_conv_con = torch.relu(self.rgb_head_convs[3](rgb_conv))
        rgb_conv_incon = torch.relu(self.rgb_head_convs[4](rgb_conv))
        ...
        dvs_conv_con = torch.relu(self.dvs_head_convs[3](dvs_conv))
        dvs_conv_incon = torch.relu(self.dvs_head_convs[4](dvs_conv))

        return rgb_conv_con,rgb_conv_incon, dvs_conv_con,dvs_conv_incon





    def forward(self, obs, detach=False,tf=False):
        # import pdb
        # pdb.set_trace()
        rgb_conv_con,rgb_conv_incon, dvs_conv_con,dvs_conv_incon = self.forward_conv(obs)

        if detach:
            rgb_conv_con = rgb_conv_con.detach()
            dvs_conv_con = dvs_conv_con.detach()
            rgb_conv_incon = rgb_conv_incon.detach()
            dvs_conv_incon = dvs_conv_incon.detach()

        # rgb_h = rgb_conv_con.view(rgb_conv_con.size(0), -1)
        rgb_h = rgb_conv_con.reshape(rgb_conv_con.size(0), -1)
        rgb_h_con = self.rgb_ln(self.rgb_fc(rgb_h))

        # dvs_h = dvs_conv_con.view(dvs_conv_con.size(0), -1)
        dvs_h = dvs_conv_con.reshape(dvs_conv_con.size(0), -1)
        dvs_h_con = self.dvs_ln(self.dvs_fc(dvs_h))


        # rgb_h_incon = rgb_conv_incon.view(rgb_conv_incon.size(0), -1)
        rgb_h_incon = rgb_conv_incon.reshape(rgb_conv_incon.size(0), -1)
        rgb_h_incon = self.rgb_ln_incon(self.rgb_fc_incon(rgb_h_incon))

        # dvs_h_incon = dvs_conv_incon.view(dvs_conv_incon.size(0), -1)
        dvs_h_incon = dvs_conv_incon.reshape(dvs_conv_incon.size(0), -1)
        dvs_h_incon = self.dvs_ln_incon(self.dvs_fc_incon(dvs_h_incon))
        ...
        if tf==True:
            return [rgb_h_con,dvs_h_con,rgb_h_incon,dvs_h_incon],[w_c1,w_i,w_c2,w_j]
